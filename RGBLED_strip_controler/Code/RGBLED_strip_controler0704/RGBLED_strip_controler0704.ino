...
void rAD() {
  ...
  if (analogRead(analogPins[5]) > 597 || analogRead(analogPins[5]) < 130) {
    mode = 0;
  }
  //Set the mode based on analog read from pin 5
  if (analogRead(analogPins[5]) > 130 && analogRead(analogPins[5]) < 230) {
    mode = 0;
  }
  if (analogRead(analogPins[5]) > 231 && analogRead(analogPins[5]) < 300) {
    mode = 1;
  }
  /*modes are unused
    ...
    mode = 3;}
  */
  if (analogRead(analogPins[5]) > 300 && analogRead(analogPins[5]) < 420) {
    mode = 4;
  }
  /*
  ...
  */
  if (analogRead(analogPins[5]) > 421 && analogRead(analogPins[5]) < 596) {
    mode = 6;
  }
}
